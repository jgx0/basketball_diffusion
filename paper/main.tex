...
(iv) a vulnerability protocol that quantifies tactical exposure through roller
openness, paint pressure, and an expected-possession-value proxy. The
complete pipeline runs end-to-end on a single GPU and is released with a
Colab notebook, unit tests, and a behavioral simulator that doubles as an
...
\item \textbf{A vulnerability protocol.} Matched-branch counterfactuals
  (scheme mutation, handler swap) scored by roller openness, paint pressure,
  and an expected-possession-value proxy; a transparent behavioral simulator
  supplies interpretable baseline contrasts and label balance
...
weight decay 0.01, batch 64; EMA 0.999; gradient-norm clip 1.0. Reference
runs: 30--40 epochs on CPU (Apple silicon) at $\sim$5\,s/epoch for the
2{,}000-possession corpus; the Colab recipe (T4 GPU, 60 epochs, mixed corpus)
is the paper-scale path. Checkpoints save every epoch; training resumes by
rerun. All 18 unit tests cover the loader, frame math, screen/label
heuristics, physics terms, and sampler contract (shape, finiteness, anchor
pinning, guidance sensitivity).

\section{Results}
\label{sec:results}

Numbers below come from the \emph{reference run} reproducible via the
repository (\texttt{make data \&\& make train \&\& make evaluate}; 30 epochs,
2{,}000 synthetic + validation-game real windows). We emphasize provenance:
these are framework-validation results, not headline NBA-scale claims, which
require the multi-game GPU corpus of \S\ref{sec:impl}.
...
train path on real tensors.

\subsection{Realism of generated possessions}
\label{sec:res-realism}

Table~\ref{tab:realism} compares generators on the validation split. The
diffusion model converges cleanly under the stabilized physics recipe
(val noise-MSE $1.04 \rightarrow 0.23$ over 30 epochs) and improves FTD by
two orders of magnitude over the mid-training baseline, with anchor-frame
pinning exact (max frame-0 error $= 0$ by construction). Violation rates
remain elevated relative to real data at this training budget --- honest
evidence that kinematic plausibility is the binding constraint at 30 epochs,
and the motivation for the guidance mechanism of \S\ref{sec:guidance}.

\begin{table}[t]
\centering
\caption{Realism on the validation split (reference run; 30 epochs). Real
data compliance reflects residual tracking noise after cleaning.}
\label{tab:realism}
\begin{tabular}{lcccc}
\toprule
...
Diffusion, epoch 30 & \textbf{14.6} & \textbf{2.57} & 0.51 & 0.014 \\
\midrule
...
Table~\ref{tab:cf} reports branch statistics. The learned model reproduces
the \emph{direction} of the simulator's ground-truth contrasts: drop yields
the most roller openness (big sagging toward the paint leaves the roll lane
spacious), blitz compresses paint pressure most (two defenders at the ball)
but concedes the most EPV proxy (pass-out advantage), and switch sits
between. The star-handler branch increases EPV proxy relative to role
handler, matching the simulator's designed gravity effect. We stress that
absolute magnitudes at this training budget are not yet calibrated to NBA
defenses; the claim validated here is \emph{conditioned sensitivity} ---
the model's distributions move measurably and in the right direction under
label interventions, which is the precondition for any tactical inference.
...
\begin{tabular}{lccc}
\toprule
Branch & Roller openness $\uparrow$ & Paint pressure (m) & EPV proxy \\
\midrule
Drop & 0.269 & 9.82 & 0.1136 \\
Switch & 0.231 & 9.75 & 0.1122 \\
Blitz & 0.216 & 9.80 & 0.1138 \\
\midrule
Role handler & 0.247 & --- & 0.1113 \\
Star handler & 0.208 & --- & 0.1158 \\
\bottomrule
\end{tabular}
\end{table}
...
artifact) show the qualitative signatures the metrics detect: under
\emph{drop}, the screener's defender sinks below the free-throw line while
the handler turns the corner into a pulling defense; under \emph{blitz}, both
ball-side defenders converge above the level of the screen and the ball
relocates; under \emph{switch}, assignments cross at contact and the big
finds himself on an island against the handler. The anchor frames are
pixel-identical across branches --- the counterfactual contract --- while
futures diverge scheme-dependently.
...
evaluations per future; large-$M$ branch studies are GPU-bound.
Consistency-style distillation is the obvious accelerator.
...
protocol --- combine into a system whose counterfactual branches are
anchor-identical, scheme-sensitive, and kinematically disciplined. Three
