...
diffusion model converges cleanly under the stabilized physics recipe in both
runs (val noise-MSE $1.04 \rightarrow 0.23$ over 30 CPU epochs;
$0.96 \rightarrow 0.08$ over 150 GPU epochs, with the physics term falling
from 348 to 1.6 during training) and improves FTD from two orders of
magnitude above the converged reference to 14.6 (CPU) and 6.19 (GPU), with
anchor-frame pinning exact (max frame-0 error $= 0$ by construction). At the
GPU budget the diffusion model's ADE (1.66\,m) undercuts the behavioral
simulator baseline (1.87\,m): the learned generator is already more faithful
to held-out futures than the hand-written tactic model on this corpus.
...
Table~\ref{tab:cf} reports branch statistics for both runs. The learned model
reproduces the \emph{direction} of the simulator's ground-truth contrasts in
both executions: drop yields the most roller openness (the big sagging toward
the paint leaves the roll lane spacious; CPU 0.269, GPU 0.425), blitz
compresses paint pressure most (two defenders at the ball) while conceding
the most EPV proxy among scheme branches on CPU (pass-out advantage), and
switch sits between on CPU. The star-handler branch lowers roller openness
(defenses shrink earlier against elite gravity) while its EPV contrast is
budget-limited on GPU; we treat the star contrast as inconclusive at the
current mixture and expect it to sharpen as real windows dominate. We stress
that absolute magnitudes are not yet calibrated to NBA defenses; the claim
validated here is \emph{conditioned sensitivity} --- the model's distributions
move measurably and in the right direction under label interventions, in two
independent executions, which is the precondition for any tactical inference.

\begin{table}[t]
\centering
\caption{Counterfactual branch statistics (same anchors; 16 samples/branch;
CPU reference run and GPU run).}
\label{tab:cf}
\begin{tabular}{lcccccc}
\toprule
 & \multicolumn{3}{c}{CPU reference run} & \multicolumn{3}{c}{GPU run} \\
\cmidrule(lr){2-4}\cmidrule(lr){5-7}
Branch & Open.$\uparrow$ & Paint (m) & EPV & Open.$\uparrow$ & Paint (m) & EPV \\
\midrule
Drop & 0.269 & 9.82 & 0.1136 & 0.425 & 10.00 & 0.0915 \\
Switch & 0.231 & 9.75 & 0.1122 & 0.327 & 9.99 & 0.0902 \\
Blitz & 0.216 & 9.80 & 0.1138 & 0.345 & 10.00 & 0.0913 \\
\midrule
Role handler & 0.247 & --- & 0.1113 & 0.303 & --- & 0.0924 \\
Star handler & 0.208 & --- & 0.1158 & 0.264 & --- & 0.0896 \\
\bottomrule
\end{tabular}
\end{table}
